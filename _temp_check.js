  ...
  function fixSidebar() {
    ...
    // Fungsi utama toggle — mobile vs desktop
        navs.forEach(function(n){
          n.classList.toggle('gt-nav-hidden', !anyHidden);
          n.classList.toggle('hidden-footer', !anyHidden);
        });
        document.body.classList.toggle('gt-nav-is-hidden', !anyHidden);
        // Update ikon hamburger
        var btn = document.getElementById('gt-sidebar-toggle-btn');
        if (btn) {
          var ic = btn.querySelector('i');
          if (ic) ic.className = anyHidden ? 'fa-solid fa-bars' : 'fa-solid fa-xmark';
        }
        try{ localStorage.setItem('sidebar_state_mobile', anyHidden ? 'visible' : 'hidden'); }catch(_){}
      } else {
        // Desktop: toggle sidebar collapse
        document.body.classList.toggle('sidebar-collapsed');
        var ic = document.body.classList.contains('sidebar-collapsed');
        try{ localStorage.setItem('sidebar_state', ic?'hidden':'visible'); }catch(_){}
      }
    };
