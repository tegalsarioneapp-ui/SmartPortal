...
<div id="hidden-notulen-container" style="display: none;">
    <div class="letter-paper" id="notulen-paper">
        <div class="kop-surat-resmi"><img src="/Lambang_Kota_Semarang.png" style="width: 70px; height: auto; filter: grayscale(100%);"><div style="flex: 1; text-align: center;"><h2 style="margin: 0; font-size: 1.4rem; font-weight: bold; text-transform: uppercase; color: black;">PENGURUS RUKUN TETANGGA 005 RUKUN WARGA 012</h2><h3 style="margin: 2px 0; font-size: 1.1rem; font-weight: normal; text-transform: uppercase; color: black;">KELURAHAN TEGALSARI â€¢ KECAMATAN CANDISARI</h3><h3 style="margin: 2px 0; font-size: 1.1rem; font-weight: normal; text-transform: uppercase; color: black;">KOTA SEMARANG</h3></div></div>
        <div style="border-top: 3px solid black; border-bottom: 1px solid black; height: 1px; margin-bottom: 20px; padding-top: 2px;"></div>
        <h3 style="text-align:center; text-decoration:underline; font-weight:bold; color:black; margin-bottom: 25px;">NOTULENSI RAPAT</h3>
        <table style="width:100%; font-size:1.1rem; color:black; border:none; margin-bottom: 25px;"><tr><td style="width:150px; padding:3px 0; border:none; background:transparent;">Hari, Tanggal</td><td style="width:20px; border:none; background:transparent;">:</td><td style="border:none; background:transparent;"><b id="cetak_n_tgl"></b></td></tr><tr><td style="padding:3px 0; border:none; background:transparent;">Agenda Utama</td><td style="border:none; background:transparent;">:</td><td style="border:none; background:transparent;"><b id="cetak_n_judul"></b></td></tr></table>
        <div id="cetak_n_isi" style="line-height:1.6; color:black; text-align:justify; margin-bottom: 40px; min-height: 200px;"></div>
        <div style="margin-top: 40px; text-align: right; font-size:1.1rem; color:black; padding-right: 20px;">Mengetahui,<br>Ketua RT. 005 / RW. 012<br><br><br><br><br><b id="cetak_n_nama_rt">Bapak Kasimin</b></div>
    </div>
</div>

<div id="print-ba-container" style="display: none !important;"></div>

<div id="modal-profil" class="modal-overlay">
    <div class="modal-content">
        <span class="close-modal" onclick="closeModalProfil()"><i class="fa-solid fa-circle-xmark"></i></span>
        <div style="text-align: center; margin-bottom: 25px;"><div style="width: 80px; height: 80px; border-radius: 50%; background: var(--primary-blue); color: white; display: flex; align-items: center; justify-content: center; font-size: 2rem; margin: 0 auto 15px auto;"><i class="fa-solid fa-user-tie"></i></div><h2 id="prof-nama" style="margin:0; font-size:1.5rem; font-weight:800; color:var(--text-dark);">Nama</h2><p style="margin:0; color:var(--primary-dark); font-weight:700; letter-spacing:1px; text-transform:uppercase; font-size:0.85rem;" id="prof-pekerjaan">Pekerjaan</p></div>
        <div style="background:#f8fafc; padding:25px; border-radius:16px; font-size:1.05rem; border:1px solid var(--border-light);"><p style="margin:8px 0; color:var(--text-muted); font-weight:600;">NIK: <span id="prof-nik" style="color:var(--text-dark); font-weight:700;"></span> | KK: <span id="prof-kk" style="color:var(--text-dark); font-weight:700;"></span></p><p style="margin:8px 0;"><i class="fa-solid fa-map-location-dot" style="color:var(--danger); width:25px; font-size:1.2rem;"></i> <span id="prof-alamat" style="color:var(--text-dark); font-weight:700;"></span></p><p style="margin:8px 0;"><i class="fa-brands fa-whatsapp" style="color:var(--success); width:25px; font-size:1.2rem;"></i> <span id="prof-telp" style="color:var(--text-dark); font-weight:700;"></span></p><hr style="border:0; border-top:1px dashed var(--border-light); margin:20px 0;"><p style="margin:8px 0; color:var(--text-muted); font-weight:600;">Istri: <span id="prof-istri" style="color:var(--text-dark); font-weight:700;"></span></p><p style="margin:8px 0; color:var(--text-muted); font-weight:600;">Anak: <span id="prof-anak" style="color:var(--text-dark); font-weight:700;"></span></p></div>
    </div>
</div>

<div id="modal-baca" class="modal-overlay">
    <div class="modal-content" style="max-width: 650px; padding: 0; overflow: hidden; display: flex; flex-direction: column; max-height: 90vh;">
        <div style="background: linear-gradient(135deg, var(--primary-blue), var(--primary-dark)); padding: 20px 30px; display: flex; justify-content: space-between; align-items: center;"><h3 style="margin: 0; color: white;"><i class="fa-solid fa-book-open-reader"></i> Ruang Baca</h3><span class="close-modal" onclick="closeModalBaca()" style="position: relative; top: 0; right: 0; color:white;"><i class="fa-solid fa-circle-xmark"></i></span></div>
        <div id="baca-content" style="padding: 30px; overflow-y: auto;"></div>
    </div>
</div>
...
<script>
    ...
    // ===== THEME (light/dark) bootstrap â€” injected by audit overhaul =====
    (function(){
        try {
    ...
    // ===== Citizen super-app dashboard injector â€” v2 =====
    (function(){
        ...
        function buildSuratStatus(){
            ...
            return '<div class="gt-status-head"><div><h4 class="gt-status-title"><i class="fa-solid fa-envelope-circle-check" style="color:var(--gt-primary-600);"></i>'+escapeHtml(keperluan)+'</h4><div class="gt-status-meta">Diajukan '+fmtDate(item.tgl||item.tanggal)+(item.no_surat?' â€¢ No. '+escapeHtml(item.no_surat):'')+'</div></div><span class="badge '+(cur===3?'badge-masuk':cur===0?'badge-menunggu':'badge-menunggu')+'">'+escapeHtml(item.status||'Diproses')+'</span></div>'
        ...
        function buildAduanStatus(){
            ...
            return '<div class="gt-status-head"><div><h4 class="gt-status-title"><i class="fa-solid fa-bullhorn" style="color:var(--gt-danger-500);"></i>'+escapeHtml(judul)+'</h4><div class="gt-status-meta">Dilaporkan '+fmtDate(it.tgl||it.tanggal)+' â€¢ Kategori: '+escapeHtml(it.kategori||'-')+'</div></div><span class="badge '+(cur===2?'badge-masuk':'badge-menunggu')+'">'+escapeHtml(it.status||'Diproses')+'</span></div>'
                + renderTimeline(steps, cur);
        }
        ...
        if (typeof origOpen === 'function' && !origOpen.__gtWrapped){
            ...
            // openWargaTab not yet defined â€” wrap once it appears
            var tries = 0;
    ...
    // ===== Notification Center & Global Warga Search â€” v1 =====
    (function(){
        function safeJSON(k, fb){ try { var v = localStorage.getItem(k); return v ? JSON.parse(v) : fb; } catch(_){ return fb; } }
        ...
        function buildNotifications(){
            ...
            aduan.forEach(function(a){
                out.push({
                    ...
                    sub: (a.namaPelapor ? a.namaPelapor + ' â€¢ ' : '') + (a.kategori || '') + ' â€¢ Status: ' + (a.status || 'Menunggu'),
                    ts: a.id || Date.now(),
                    onClick: function(){
            ...
            if (Array.isArray(surat)) {
                surat.forEach(function(s, i){
                    ...
                    out.push({
                        ...
                        sub: 'Status: ' + (s.status || 'Diproses') + (s.no_surat ? ' â€¢ '+s.no_surat : ''),
        ...
        function wireBell(){
            ...
            // Strip the legacy onclick (Swal alert) â€” use property reset, not removeAttribute (would re-fire on reload)
                ...
                if (current.length === 0) {
                    ...
                    current.forEach(function(w, i){
                        ...
                        html += '<div class="gt-search-row" data-idx="'+i+'">'
                            ...
                            +   '<div class="gt-sr-meta">NIK '+highlight(w.nik || '-', q)+' â€¢ '+highlight(w.alamat || '-', q)+'</div>'
                            + '</div></div>';
                    });
                    html += '<div class="gt-search-foot"><span>'+current.length+' hasil</span><span><kbd>â†‘</kbd><kbd>â†“</kbd> navigasi â€¢ <kbd>Enter</kbd> buka â€¢ <kbd>Esc</kbd> tutup</span></div>';
                    box.innerHTML = html;
    ...
    // ===== Mobile sticky bottom nav (role-aware) â€” v1 =====
    (function(){
        var WARGA_NAV = [
    ...
    /* Hide & Seek pull-tab dipulihkan â€” IIFE "unhideNav" yang dulu
    ...
    // ============================================================
    // PDF DOWNLOAD HELPER (mengganti seluruh "Cetak" â†’ Download PDF)
    ...
    window.downloadPdfFromHtml = async function(htmlBody, title){
        ...
        // Catat riwayat ekspor (selalu, bahkan jika user batal â€” supaya konsisten dgn perilaku lama)
        try {
        ...
        // Rewrite path img/asset relatif â†’ absolute (Lambang_Kota_Semarang.png dll)
        ...
        // Buat container offscreen lebar 794px (â‰ˆ A4 96dpi)
...
<table class="ttd-table">
  ...
  <tr>
    ...
    // === MESIN ANALISA & PENAGIHAN REAL-TIME (DIBEDAH 3 POS) ===
    window.tampilRiwayatEkspor = function() {
        var rw = JSON.parse(localStorage.getItem('db_riwayat_ekspor')) || [];
        rw.sort(function(a,b){ return (b.ts||0) - (a.ts||0); });
        var rows = rw.length ? rw.map(function(r,i){
            return '<tr>' +
                '<td style="padding:6px 8px; border-bottom:1px solid #e2e8f0; text-align:center;">' + (i+1) + '</td>' +
                '<td style="padding:6px 8px; border-bottom:1px solid #e2e8f0; white-space:nowrap;">' + (r.tgl || '-') + '</td>' +
                '<td style="padding:6px 8px; border-bottom:1px solid #e2e8f0; word-break:break-word;">' + (r.judul || '-') + '</td>' +
                '<td style="padding:6px 8px; border-bottom:1px solid #e2e8f0; text-align:center;"><button class="btn-action" style="padding:4px 8px; font-size:0.75rem;" onclick="hapusRiwayatEkspor(' + r.id + ')" title="Hapus entri"><i class="fa-solid fa-trash"></i></button></td>' +
            '</tr>';
        }).join('') : '<tr><td colspan="4" style="padding:20px; text-align:center; color:#64748b;"><i class="fa-solid fa-folder-open"></i> Belum ada dokumen yang diekspor.</td></tr>';
        var html = '<div style="max-height:55vh; overflow:auto; text-align:left; border:1px solid #cbd5e1; border-radius:8px;">' +
            '<table style="width:100%; border-collapse:collapse; font-size:0.9rem;">' +
            '<thead><tr style="background:#f1f5f9; position:sticky; top:0;"><th style="padding:8px;">No</th><th style="padding:8px; text-align:left;">Tanggal</th><th style="padding:8px; text-align:left;">Nama Dokumen</th><th style="padding:8px; width:60px;">Aksi</th></tr></thead>' +
            '<tbody>' + rows + '</tbody></table></div>' +
            '<div style="margin-top:10px; font-size:0.8rem; color:#64748b;">Total: <b>' + rw.length + '</b> ekspor tercatat</div>';
        Swal.fire({
            title: '<i class="fa-solid fa-clock-rotate-left"></i> Riwayat Ekspor Dokumen',
            html: html, width: 720,
            showCancelButton: true, cancelButtonText: 'Tutup',
            confirmButtonText: '<i class="fa-solid fa-broom"></i> Bersihkan Semua',
            confirmButtonColor: '#ef4444',
        }).then(function(res){
            if (res.isConfirmed) {
                Swal.fire({ title:'Hapus seluruh riwayat?', icon:'warning', showCancelButton:true, confirmButtonColor:'#ef4444' }).then(function(r){
                    if (r.isConfirmed) {
                        localStorage.setItem('db_riwayat_ekspor', '[]');
                        localStorage.setItem('ts_riwayat_ekspor', new Date().toISOString());
                        if (typeof syncSemuaData === 'function') { try { syncSemuaData(true); } catch(e){} }
                        Toast.fire({ icon:'success', title:'Riwayat dibersihkan' });
                    }
                });
            }
        });
    };
    window.hapusRiwayatEkspor = function(id) {
        var rw = JSON.parse(localStorage.getItem('db_riwayat_ekspor')) || [];
        rw = rw.filter(function(x){ return String(x.id) !== String(id); });
        localStorage.setItem('db_riwayat_ekspor', JSON.stringify(rw));
        localStorage.setItem('ts_riwayat_ekspor', new Date().toISOString());
        if (typeof syncSemuaData === 'function') { try { syncSemuaData(true); } catch(e){} }
        window.tampilRiwayatEkspor();
    };
    window.cetakAnalisaIuran = function() {
        try {
            var dbIuran = JSON.parse(localStorage.getItem('db_iuran')) || [];
            var dbWarga = JSON.parse(localStorage.getItem('db_warga')) || [];
            var validWarga = dbWarga.filter(function(w){ return w && w.nama && String(w.nama).trim() !== ''; });
            var bulanSkg = new Intl.DateTimeFormat('id-ID', { month: 'long' }).format(new Date());
            var tahunSkg = new Date().getFullYear();
            var jmlBayar = dbIuran.length;
            var totalWarga = validWarga.length;
            var mejaTerkumpul = jmlBayar * 5000, targetMeja = 250000, mejaKurang = Math.max(0, targetMeja - mejaTerkumpul);
            var bangunTerkumpul = jmlBayar * 10000, bangunTarget = totalWarga * 10000, bangunKurang = Math.max(0, bangunTarget - bangunTerkumpul);
            var agustusTerkumpul = jmlBayar * 5000, agustusTarget = totalWarga * 5000, agustusKurang = Math.max(0, agustusTarget - agustusTerkumpul);
            var rowsHtml = '';
            ...
            sorted.forEach(function(w, i){
                var bayar = dbIuran.find(function(it){ return String(it.idWarga) === String(w.id) && it.bulan === bulanSkg; });
                rowsHtml += '<tr>' +
                    '<td style="text-align:center; border:1px solid #333; padding:5px;">' + (i+1) + '</td>' +
                    '<td style="border:1px solid #333; padding:5px;">' + w.nama + '</td>' +
                    '<td style="text-align:right; border:1px solid #333; padding:5px;">' + (bayar ? fmt(bayar.nominal) : 'Rp 0') + '</td>' +
                    '<td style="text-align:center; border:1px solid #333; padding:5px;">' + (bayar ? 'LUNAS' : 'BELUM') + '</td>' +
                '</tr>';
            });
            if (!rowsHtml) rowsHtml = '<tr><td colspan="4" style="text-align:center; border:1px solid #333; padding:8px;"><i>Belum ada data warga.</i></td></tr>';
            var tgl = new Date().toLocaleDateString('id-ID', { day:'2-digit', month:'long', year:'numeric' });
            var html = '' +
                '<div class="kop-surat-resmi" style="display:flex; align-items:center; gap:15px; border-bottom:3px double #000; padding-bottom:10px; margin-bottom:15px;">' +
                  '<img src="/Lambang_Kota_Semarang.png" alt="Logo Semarang" style="width:80px; height:auto;">' +
                  '<div style="text-align:center; flex:1;">' +
                    '<div style="font-size:13pt; font-weight:bold;">PENGURUS RT 005 / RW 012</div>' +
                    '<div style="font-size:12pt; font-weight:bold;">KELURAHAN TEGALSARI - KOTA SEMARANG</div>' +
                    '<div style="font-style:italic; font-size:10pt;">Sekretariat: Jl. Tegalsari Raya, Kota Semarang, Jawa Tengah</div>' +
                  '</div>' +
                '</div>' +
                '<h3 style="text-align:center; margin:10px 0 4px 0;">RINGKASAN ANALISA & PEMECAHAN IURAN</h3>' +
                '<p style="text-align:center; margin:0 0 15px 0;">Periode: <b>' + bulanSkg + ' ' + tahunSkg + '</b> &mdash; Dicetak: ' + tgl + '</p>' +
                '<table style="width:100%; border-collapse:collapse; margin-bottom:15px;">' +
                  '<thead><tr style="background:#f0f0f0;">' +
                    '<th style="border:1px solid #333; padding:6px;">Pos Iuran</th>' +
                    '<th style="border:1px solid #333; padding:6px;">Terkumpul</th>' +
                    '<th style="border:1px solid #333; padding:6px;">Target</th>' +
                    '<th style="border:1px solid #333; padding:6px;">Kekurangan</th>' +
                  '</tr></thead><tbody>' +
                    '<tr><td style="border:1px solid #333; padding:6px;">Pos Uang Meja</td>' +
                        '<td style="border:1px solid #333; padding:6px; text-align:right;">' + fmt(mejaTerkumpul) + '</td>' +
                        '<td style="border:1px solid #333; padding:6px; text-align:right;">' + fmt(targetMeja) + '</td>' +
                        '<td style="border:1px solid #333; padding:6px; text-align:right;">' + (mejaKurang>0?fmt(mejaKurang):'LUNAS') + '</td></tr>' +
                    '<tr><td style="border:1px solid #333; padding:6px;">Pembangunan</td>' +
                        '<td style="border:1px solid #333; padding:6px; text-align:right;">' + fmt(bangunTerkumpul) + '</td>' +
                        '<td style="border:1px solid #333; padding:6px; text-align:right;">' + fmt(bangunTarget) + '</td>' +
                        '<td style="border:1px solid #333; padding:6px; text-align:right;">' + (bangunKurang>0?fmt(bangunKurang):'LUNAS') + '</td></tr>' +
                    '<tr><td style="border:1px solid #333; padding:6px;">17 Agustus</td>' +
                        '<td style="border:1px solid #333; padding:6px; text-align:right;">' + fmt(agustusTerkumpul) + '</td>' +
                        '<td style="border:1px solid #333; padding:6px; text-align:right;">' + fmt(agustusTarget) + '</td>' +
                        '<td style="border:1px solid #333; padding:6px; text-align:right;">' + (agustusKurang>0?fmt(agustusKurang):'LUNAS') + '</td></tr>' +
                    '<tr style="background:#fff7e6; font-weight:bold;"><td style="border:1px solid #333; padding:6px;">TOTAL SETORAN MASUK</td>' +
                        '<td colspan="3" style="border:1px solid #333; padding:6px; text-align:right;">' + jmlBayar + ' setoran &times; Rp 20.000 = ' + fmt(jmlBayar*20000) + '</td></tr>' +
                  '</tbody></table>' +
                '<h4 style="margin:15px 0 6px 0;">Daftar Penagihan Bulan Berjalan (' + bulanSkg + ')</h4>' +
                '<table style="width:100%; border-collapse:collapse; font-size:10pt;">' +
                  '<thead><tr style="background:#f0f0f0;">' +
                    '<th style="border:1px solid #333; padding:5px; width:40px;">No</th>' +
                    '<th style="border:1px solid #333; padding:5px; text-align:left;">Nama Warga</th>' +
                    '<th style="border:1px solid #333; padding:5px; width:130px;">Nominal</th>' +
                    '<th style="border:1px solid #333; padding:5px; width:90px;">Status</th>' +
                  '</tr></thead><tbody>' + rowsHtml + '</tbody></table>' +
                '<div style="display:flex; justify-content:flex-end; margin-top:35px;">' +
                  '<div style="text-align:center;">' +
                    '<div>Semarang, ' + tgl + '</div>' +
                    '<div style="margin-bottom:55px;">Bendahara RT 005 / RW 012</div>' +
                    '<div style="border-top:1px solid #000; padding-top:4px; min-width:180px;"><b>(.................................)</b></div>' +
                  '</div>' +
                '</div>';
            window.printViaIframe(html, 'Ringkasan Analisa Iuran - ' + bulanSkg + ' ' + tahunSkg);
        } catch(err) {
            console.error(err);
            Swal.fire('Gagal', 'Tidak dapat menyiapkan dokumen: ' + err.message, 'error');
        }
    };
    window.loadAnalisaUangMeja = function() {
        let dbIuran = JSON.parse(localStorage.getItem('db_iuran')) || [];
        let dbWarga = JSON.parse(localStorage.getItem('db_warga')) || [];
        let validWarga = dbWarga.filter(w => w && w.nama && String(w.nama).trim() !== '');
        
        let bulanSkg = new Intl.DateTimeFormat('id-ID', { month: 'long' }).format(new Date());
        if(document.getElementById('bulan-tagihan-info')) document.getElementById('bulan-tagihan-info').innerText = "Periode: " + bulanSkg;

        let bayarBulanIni = dbIuran;
        // Hitung total bulan-orang setoran (mis. 1 warga bayar 2 bulan = 2 setoran).
        let jmlBayar = dbIuran.length;
        let totalWarga = validWarga.length;

        let st = JSON.parse(localStorage.getItem('status_setor_' + bulanSkg)) || { meja:0, bangun:0, agustus:0 };
        const getBadgeHTML = (val) => {
            if(val === 2) return `<span class="badge badge-masuk" style="background:#dcfce7; color:#166534;"><i class="fa-solid fa-check-double"></i> Diterima Valid</span>`;
            if(val === 1) return `<span class="badge badge-menunggu" style="background:#fef08a; color:#854d0e;"><i class="fa-solid fa-clock"></i> Proses Setor</span>`;
            return `<span class="badge" style="background:#f1f5f9; color:#64748b; border:1px solid #cbd5e1;"><i class="fa-solid fa-xmark"></i> Belum Setor</span>`;
        };

        // 1. UANG MEJA
        let mejaTerkumpul = jmlBayar * 5000; let targetMeja = 250000; let mejaKurang = targetMeja - mejaTerkumpul;
        if(document.getElementById('tagih-meja-terkumpul')) document.getElementById('tagih-meja-terkumpul').innerText = fmt(mejaTerkumpul);
        if(document.getElementById('tagih-meja-peserta')) document.getElementById('tagih-meja-peserta').innerText = `${jmlBayar} Setoran`;
        if(document.getElementById('tagih-meja-kurang')) {
            document.getElementById('tagih-meja-kurang').innerText = mejaKurang > 0 ? fmt(mejaKurang) : "LUNAS";
            document.getElementById('btn-nalangi').style.display = (mejaKurang > 0 && mejaTerkumpul > 0) ? 'inline-flex' : 'none';
            window.currentDefisitMeja = mejaKurang > 0 ? mejaKurang : 0;
            window.currentTerkumpulMeja = mejaTerkumpul;
        }
        if(document.getElementById('badge-status-meja')) document.getElementById('badge-status-meja').innerHTML = getBadgeHTML(st.meja);

        // 2. PEMBANGUNAN
        let bangunTerkumpul = jmlBayar * 10000; let bangunTarget = totalWarga * 10000; let bangunKurang = bangunTarget - bangunTerkumpul;
        if(document.getElementById('tagih-bangun-terkumpul')) document.getElementById('tagih-bangun-terkumpul').innerText = fmt(bangunTerkumpul);
        if(document.getElementById('tagih-bangun-kurang')) document.getElementById('tagih-bangun-kurang').innerText = fmt(bangunKurang > 0 ? bangunKurang : 0);
        if(document.getElementById('badge-status-bangun')) document.getElementById('badge-status-bangun').innerHTML = getBadgeHTML(st.bangun);

        // 3. 17 AGUSTUS
        let agustusTerkumpul = jmlBayar * 5000; let agustusTarget = totalWarga * 5000; let agustusKurang = agustusTarget - agustusTerkumpul;
        if(document.getElementById('tagih-agustus-terkumpul')) document.getElementById('tagih-agustus-terkumpul').innerText = fmt(agustusTerkumpul);
        if(document.getElementById('tagih-agustus-kurang')) document.getElementById('tagih-agustus-kurang').innerText = fmt(agustusKurang > 0 ? agustusKurang : 0);
        if(document.getElementById('badge-status-agustus')) document.getElementById('badge-status-agustus').innerHTML = getBadgeHTML(st.agustus);

        // MENGGAMBAR DAFTAR KE ID BARU YANG UNIK
        let tb = document.getElementById('tbody-daftar-warga-final'); 
        if(!tb) return; 
        tb.innerHTML = '';

        if(validWarga.length === 0) {
            tb.innerHTML = '<tr><td colspan="4" style="text-align:center;">Data warga kosong.</td></tr>';
            return;
        }

        validWarga.sort((a,b) => String(a.nama).localeCompare(String(b.nama))).forEach(w => {
            let dataBayar = dbIuran.find(i => String(i.idWarga) === String(w.id) && i.bulan === bulanSkg);
            let statusBadge = dataBayar ? `<span class="badge badge-masuk"><i class="fa-solid fa-check-circle"></i> LUNAS</span>` : `<span class="badge badge-menunggu"><i class="fa-solid fa-clock"></i> BELUM BAYAR</span>`;
            
            let rincianHTML = dataBayar 
                ? `<div style="font-size:0.85rem; color:var(--text-muted); margin-top:6px;"><i class="fa-solid fa-check" style="color:var(--success)"></i> Pembangunan: 10k | Meja: 5k | 17 Agt: 5k</div>` 
                : `<div style="font-size:0.85rem; color:var(--danger); margin-top:6px;">Belum ada setoran bulan ini</div>`;
            
            let tombolAksi = dataBayar 
                ? `<button class="btn-action bg-blue" onclick="lihatKartuWarga(${w.id})" style="font-size:0.8rem; padding:6px 12px;"><i class="fa-solid fa-id-card"></i> Lihat Kartu</button>` 
                : `<button class="btn-action bg-gold" onclick="bayarCepatTagihan(${w.id}, '${w.nama.replace(/'/g,"\\'")}')" style="font-size:0.85rem; padding:8px 12px; font-weight:900;"><i class="fa-solid fa-hand-holding-dollar"></i> Bayar 20k</button>`;
            
            tb.innerHTML += `<tr class="row-tagihan-unik">
                <td><b class="nama-warga-pencarian" style="font-size:1.05rem;">${w.nama}</b></td>
                <td><b style="color:var(--primary-dark); font-size:1.1rem;">${dataBayar ? fmt(dataBayar.nominal) : 'Rp 0'}</b>${rincianHTML}</td>
                <td>${statusBadge}</td>
    ...
    // FUNGSI PENCARIAN (ANTI MACET)
    // === FUNGSI PENCARIAN CERDAS (ANTI MACET / ID KEMBAR) ===
    ...
    window.loadPengaturan = function() {
        ...
        ['admin','bendahara','koperasi','warga'].forEach(function(role) {
            ...
            if(el) el.placeholder = _savedPw[role]
                ? '(sudah diubah â€” isi untuk ganti lagi)'
    ...
    window.refreshInfoAplikasi = async function() {
        try {
            ...
            if (elTotal) elTotal.textContent = data.rowCount !== undefined ? data.rowCount + ' baris' : 'â€”';
            if (elSSE) {
                elSSE.textContent = data.sseSubscribers !== undefined ? data.sseSubscribers + ' subscriber aktif' : 'â€”';
            }
        } catch(e) {}
        ...
        try {
            ...
            await Swal.fire({
                ...
                html: `<div style="text-align:left;font-size:0.88rem;line-height:2;">
                    <b>Driver:</b> ${d.driver || 'â€”'}<br>
                    <b>Total Baris DB:</b> ${d.rowCount !== undefined ? d.rowCount : 'â€”'}<br>
                    <b>SSE Subscriber:</b> ${d.sseSubscribers !== undefined ? d.sseSubscribers : 'â€”'}<br>
                    <b>Server Time:</b> ${d.serverTime ? new Date(d.serverTime).toLocaleString('id-ID') : 'â€”'}<br>
                    <b>Status:</b> ${d.message || 'â€”'}
                </div>`,
    ...
    window.checkPushStatus = async function() {
        ...
        try {
            ...
            _gtPushSub = sub;
            if (sub) {
                if (statusEl) statusEl.textContent = 'âœ“ Aktif â€” notifikasi akan masuk ke perangkat ini';
    ...
    window.togglePushSubscription = async function() {
        ...
        try {
            ...
            if (sub) {
                ...
                // Subscribe â€” request permission first
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        Swal.fire({
            ...
            input: 'select',
            inputOptions: {
                'warga': 'ðŸ—‘ï¸ Data Keluarga & Mutasi (KK)',
                'keuangan': 'ðŸ—‘ï¸ Data Keuangan (Kas & Iuran)',
                'koperasi': 'ðŸ—‘ï¸ Data Koperasi (Tabungan & Pinjam)',
                'persuratan': 'ðŸ—‘ï¸ Data Surat, Aduan & Agenda',
                'semua': 'âš ï¸ RESET TOTAL (Semua Data Pabrik)'
            },
...
window.loadMatriksIuran = function() {
    ...
    window.bayarCepatTagihan = function(idWarga, namaWarga) {
        let bulanSkg = new Intl.DateTimeFormat('id-ID', { month: 'long' }).format(new Date());
    ...
    // â•â•â• SISTEM KURANG BAYAR â•â•â•
    ...
    window.editDarurat = function(id) {
      ...
      window.syncSemuaData = async function(isAuto = false) {
          ...
          if(txt) { txt.innerText = 'âœ“ ' + nowSyncStr + ' WIB'; txt.style.color = 'var(--success)'; }
          setTimeout(function() {
...
window.cetakPDFSurat = function(id, isSalinan) {
    ...
    // Download langsung sebagai PDF (bukan dialog Print) â€” Riwayat Ekspor tetap tercatat.
...
// === FITUR HIDE & SEEK FOOTER GAYA MY BCA ===
    ...
    // ============================================================
    // BIOMETRIC LOGIN (WebAuthn â€” sidik jari / Face ID)
    // ============================================================
    ...
    window.loginBiometrik = async function(){
        try {
            ...
            Toast.fire({icon:'success', title:'Login biometrik berhasil â€” selamat datang '+ w.nama});
        } catch(err){
    ...
    // ============================================================
    // SESSION RESTORE â€” refresh tetap di halaman terakhir, bukan splash
    // ============================================================
    ...
    (function(){
        ...
        window.BukaPortal = function(role){
            ...
            // Setelah portal terbuka & lastView ada â†’ restore tab terakhir untuk role ini
            try {
    ...
    // Wrap logout â€” bersihkan session lokal
    (function(){
    ...
    */
    (function smartPortalSuratV3() {
      ...
      window.kirimTokenSurat = function(id) {
        ...
        var w = wargaByRecord(rec);
        var msgWA = 'Halo ' + (rec.nama||'') + ', surat pengantar Anda dengan No ' + (rec.noSurat||'-') + ' (Kode: ' + (rec.kodePengajuan||'-') + ') telah disetujui. Token download Anda: *' + rec.token + '* â€” Silakan masukkan token ini di Portal Warga untuk mengunduh surat.';
        ...
        var html = '<div style="text-align:left; font-size:0.95rem;">'
          ...
          + '<p style="color:#64748b; font-size:0.85rem;">Klik tombol di bawah untuk membuka pesan. Token siap dikirim â€” tinggal tekan kirim di aplikasi WA / email Anda.</p>'
          + '</div>';
        Swal.fire({
